...
/// \return `Ok(())` on success, `Err` on denial, a full pool, or
///         hardware failure.
pub fn generate(name: &str, curve: Curve) -> Result<()> {
    let c = CString::new(name).map_err(|_| Error::InvalidArg)?;
    check(unsafe { host_ecc_generate(c.as_ptr(), curve.as_u8()) })
}
...
pub fn import(name: &str, priv_key: &[u8], curve: Curve) -> Result<()> {
    let c = CString::new(name).map_err(|_| Error::InvalidArg)?;
    check(unsafe { host_ecc_import(c.as_ptr(), priv_key.as_ptr(), curve.as_u8()) })
}
...
/// \return The raw public key, or `Err` on failure.
pub fn pubkey(name: &str, curve: Curve) -> Result<Vec<u8>> {
    let c = CString::new(name).map_err(|_| Error::InvalidArg)?;
...
pub fn delete(name: &str) -> Result<()> {
    let c = CString::new(name).map_err(|_| Error::InvalidArg)?;
    check(unsafe { host_ecc_delete(c.as_ptr()) })
}
...
/// \return The 64-byte raw signature, or `Err` on failure.
pub fn ecdsa_sign(name: &str, msg: &[u8]) -> Result<[u8; 64]> {
    let c = CString::new(name).map_err(|_| Error::InvalidArg)?;
    let mut sig = [0u8; 64];
    check(unsafe { host_ecdsa_sign(c.as_ptr(), msg.as_ptr(), msg.len(), sig.as_mut_ptr()) })?;
    Ok(sig)
}

/// \brief Ed25519-sign `msg` with the key `name`.
/// \return The 64-byte signature, or `Err` on failure.
pub fn eddsa_sign(name: &str, msg: &[u8]) -> Result<[u8; 64]> {
    let c = CString::new(name).map_err(|_| Error::InvalidArg)?;
    let mut sig = [0u8; 64];
    check(unsafe { host_eddsa_sign(c.as_ptr(), msg.as_ptr(), msg.len(), sig.as_mut_ptr()) })?;
    Ok(sig)
}

/// \brief Read the TROPIC01 chip serial / identity blob.
/// \param max Capacity to request from the chip.
/// \return The identity bytes, or `Err` on failure.
pub fn chip_id(max: usize) -> Result<Vec<u8>> {
...
/// \return `(riscv, spect)` 4-byte version tuples, or `Err`.
pub fn fw_version() -> Result<([u8; 4], [u8; 4])> {
    let mut riscv = [0u8; 4];
    let mut spect = [0u8; 4];
    check(unsafe { host_se_fw_version(riscv.as_mut_ptr(), spect.as_mut_ptr()) })?;
    Ok((riscv, spect))
}
