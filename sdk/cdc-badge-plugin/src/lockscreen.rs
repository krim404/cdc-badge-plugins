...
/// \return `true` on success, `false` if the label key cannot be encoded
///         or the host refused the call.
pub fn register(label_key: &str, action_id: u32) -> bool {
    let Ok(k) = CString::new(label_key) else { return false; };
    unsafe { host_lockscreen_register_action(k.as_ptr(), action_id) == 0 }
}
